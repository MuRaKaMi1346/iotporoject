...
// กำหนดพินต่าง ๆ
#define DHTPIN 4          // พินเชื่อมต่อ DHT22
#define DHTTYPE DHT22     // ประเภทเซนเซอร์ DHT
#define SOIL_AO_PIN 34    // พิน Analog Output ของเซนเซอร์ความชื้นในดิน
#define SOIL_DO_PIN 27    // พิน Digital Output ของเซนเซอร์ความชื้นในดิน
...
Adafruit_SH1106G display = Adafruit_SH1106G(SCREEN_WIDTH, SCREEN_HEIGHT, &Wire, OLED_RESET);  // ใช้ SH1106G
...
BLYNK_WRITE(V6) {
  leg = param.asInt();
  pinValue = 0 ;
  analogWrite(ledPin1, 0); // ปิดหลอดไฟที่ 1
  analogWrite(ledPin2, 0); // ปิดหลอดไฟที่ 2
...
void loop() {
  ...
  // อ่านค่าความชื้นจากเซนเซอร์ดิน
  int soilMoistureAnalog = analogRead(SOIL_AO_PIN);
  int soilMoistureDigital = digitalRead(SOIL_DO_PIN);

  float percentage_Soil = 100-((float(soilMoistureAnalog) / 4095.0) * 100);

  // ตรวจสอบข้อผิดพลาดของ DHT22
  ...
  Serial.println(soilhumchoose);
  if (percentage_Soil > soilhumchoose) {
